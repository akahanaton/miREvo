...
sub get_data_without_outliers {
    ...
    if ($self->count() < $Descriptive::Min_samples_number) {
        carp("Need at least $Descriptive::Min_samples_number samples\n");
        return;
    }
...
sub _test_for_too_small_val
{
    ...
    return (abs($val) <= $Descriptive::Tolerance);
}
...
sub least_squares_fit {
  ...
  return ()
    unless abs( $denom ) > $Descriptive::Tolerance;
  ...
  return () unless (abs( $denom ) > $Descriptive::Tolerance);
...
Descriptive - Module of basic descriptive statistical functions.

=head1 SYNOPSIS

  use Descriptive;
  $stat = Descriptive::Full->new();
  $stat->add_data(1,2,3,4); $mean = $stat->mean();
  $var  = $stat->variance();
  $tm   = $stat->trimmed_mean(.25);
  $Descriptive::Tolerance = 1e-10;
...
greater than the value C<$Descriptive::Tolerance>, which
...
=item $stat = Descriptive::Sparse->new();
...
=item $stat = Descriptive::Full->new();

Create a new statistics object that inherits from
Descriptive::Sparse so that it contains all the methods
described above.
...
samples to apply the outlier filtering is C<$Descriptive::Min_samples_number>,
4 by default.
...
versions of Descriptive and using it is discouraged.
...
Which version of Descriptive you're using.  As you can
