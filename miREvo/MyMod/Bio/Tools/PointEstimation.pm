package PointEstimation;
use strict;
use Carp;
use vars qw($VERSION @ISA $AUTOLOAD);
use MyMod::Bio::Tools::Distributions; #qw(chisqrdistr tdistr fdistr udistr uprob chisqrprob tprob fprob);
use MyMod::Bio::Tools::Descriptive;
use POSIX;


@ISA= qw (Descriptive::Full);
$VERSION = '1.1';
...
sub compute_confidence_interval{
	...
	print "\there\n";
	#--------------------------------------------------
	# my $a = abs tdistr($self->{df},$self->{alpha});
	#-------------------------------------------------- 
	my $a = abs Distributions::tdistr(4,0.025);
	print $a,"\there\n";
	$self->{t_value} = $a;
	...
	$self->{t_statistic}=$self->{standard_error}
						...
						$self->{t_prob}=1- abs (Distributions::tprob($self->{df},-1*$self->{t_statistic})-Distributions::tprob($self->{df},$self->{t_statistic})) ;
	$self->{valid}=1;
	return 1;
...
package PointEstimation::Sufficient;
use strict;
use Carp;
use vars qw($VERSION $AUTOLOAD @ISA);
use POSIX;
@ISA=qw (PointEstimation);
$VERSION='1.1';
...
sub add_data{
     ...
     croak "the add_data() method is not supported in PointEstimation::Sufficient\n";
...
PointEstimation - Perl module for computing confidence intervals in parameter estimation with Student's T distribution
PointEstimation::Sufficient - Perl module for computing the confidence intervals using sufficient statistics

=head1 SYNOPSIS

  # example for PointEstimation
  use PointEstimation;
  ...
  #example for PointEstimation::Sufficient

  use strict;
  use PointEstimation;
  my ($count,$mean,$variance)=(30,3.996,1.235); 
  my $stat = new PointEstimation::Sufficient;
...
=head2  PointEstimation

  This module is a subclass of Descriptive::Full. It uses T-distribution for point estimation 
  assuming the data is normally distributed or the sample size is sufficiently large. It overrides the 
  add_data() method in Descriptive to compute the confidence interval with the specified significance
   level (default is 95%). It also computes the t-statistic=T and Prob>|T| in case of hypothesis 
  testing of paired T-tests.

=head2  PointEstimation::Sufficient

 This module is a subclass of PointEstimation. Instead of taking the real data points as the input, 
 ...
 function. The output will be exactly the same as the PointEstimation Module.
