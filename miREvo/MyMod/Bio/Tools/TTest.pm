...
use MyMod::Bio::Tools::Distributions qw (tdistr fdistr tprob fprob);
use MyMod::Bio::Tools::PointEstimation;
$VERSION='1.1';
my %fields=
(
	's1'          =>undef,     #sample 1 
...
sub load_data
{
	...
	my $s1= new PointEstimation;
	my $s2= new PointEstimation;
...
package TTest::Sufficient;
use strict;
use Carp;
use vars qw($VERSION @ISA $AUTOLOAD);
use MyMod::Bio::Tools::PointEstimation;
use MyMod::Bio::Tools::TTest;
use POSIX;
@ISA= qw (TTest);
$VERSION = '1.1';
...
sub load_data{
	my $self=shift;
        ...
        my $s1= new PointEstimation::Sufficient;
        my $s2= new PointEstimation::Sufficient;
...
=head1 NAME
 ...
 TTest - Perl module to perform T-test on 2 independent samples
 TTest::Sufficient - Perl module to perfrom T-Test on 2 indepdent samples using sufficient statistics
...
=head1 SYNOPSIS
 ...
 my $s1=$ttest->{s1};  #sample 1  a PointEstimation object
 my $s2=$ttest->{s2};  #sample 2  a PointEstimation object
 print "*****************************************************\n\n";
 $s1->output_confidence_interval('1');
 print "*****************************************************\n\n";
 $s2->output_confidence_interval('2');
...
=head2 TTest::Sufficient

 This module is a subclass of TTest. Instead of taking the real data points as the input, 
 ...
 function. The output will be exactly the same as the TTest Module.
...
Descriptive Distributions PointEstimation
