...
sub map_reads{
	...
	if(defined $options{'v'}){
		$mapping_loc=$options{'v'};
	}

	print MAP "bowtie -p 8 -f -n $mismatches_seed -e 80 -l 18 -a -m $mapping_loc --best --strata $file_genome_latest $file_reads_latest $dir/mappings.bwt\n\n";

    my $ret_mapping=`bowtie -p 8 -f -n $mismatches_seed -e 80 -l 18 -a -m $mapping_loc --best --strata $file_genome_latest $file_reads_latest $dir/mappings.bwt`;
