...
sub pass_filtering_structure{
    ...
    #not more than 3 nt difference between mature and star length, default = 6
    unless(-3<diff_lng() and diff_lng()<3){$ret=0; $hash_comp{"problem_duplex_lng"}="the difference between the candidate mature and star sequence is > 6 nts\n";}
	#maximum 5 unpaired bases
	unless (ttl_unpaired()<6) {$ret=0; $hash_comp{"problem_duplex_bp"}="too many unpaired bases in duplex\n";}
	#maximum 3 consecutive unpaired bases
	unless (consc_unpaired()<=3) {$ret=0; $hash_comp{"problem_duplex_bp"}="too long size of consecutive unpaired bases in duplex\n";}
